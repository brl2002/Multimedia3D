...
State executeRecord(FMOD::Studio::System* system)
{
    ...
    ERRCHECK( system->loadBankFile(Common_MediaPath("Master Bank.bank.strings"), FMOD_STUDIO_LOAD_BANK_NONBLOCKING, &stringsBank) );
    ...
    FMOD::Studio::ID engineID = {0};
    ERRCHECK( system->lookupID("event:/Vehicles/Basic Engine", &engineID) );

    FMOD::Studio::EventDescription* engineDescription = NULL;
    ERRCHECK( system->getEvent(&engineID, FMOD_STUDIO_LOAD_BEGIN_NOW, &engineDescription) );
