...
namespace Advanced2D {
	...
	Sprite::Sprite() : Entity(RENDER2D)
	{
		...
		this->usePortion = false;
		this->animationRects = std::vector<RECT>();
	}
	...
	void Sprite::draw()
	{
		...
		{
			RECT srcRect = animationRects[curframe];
			this->setWidth(abs(srcRect.right - srcRect.left));
			this->setHeight(abs(srcRect.top - srcRect.bottom));
		...
		{
			this->transform();
			g_engine->getSpriteHandler()->Draw(this->image->GetTexture(), &portionRect, NULL, NULL, color);
		}
		else
		{
	...
	void Sprite::setAnimationRects(std::vector<RECT> rects)
	{
		this->animationRects = rects;
		this->setWidth(abs(rects[0].right - rects[0].left));
		this->setHeight(abs(rects[0].top - rects[0].bottom));
	}

	void Sprite::setPortionRect(RECT rect)
	{
		this->usePortion = true;
		this->portionRect = rect;
		this->setWidth(abs(rect.right - rect.left));
		this->setHeight(abs(rect.top - rect.bottom));
	}
}
